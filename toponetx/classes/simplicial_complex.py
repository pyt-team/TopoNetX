...
class SimplicialComplex(Complex, Generic[ElementType]):
    ...
    **kwargs : keyword arguments, optional
    ...
    def skeleton(self, rank: int) -> list[Simplex[ElementType]]:
        ...
        """
        return sorted(map(lambda node: node.simplex, self._simplex_trie.skeleton(rank)))
    ...
    def __contains__(self, item: Iterable[ElementType]) -> bool:
        ...
        if not isinstance(item, Iterable):
            return (item,) in self._simplex_trie
        return tuple(sorted(item)) in self._simplex_trie
    ...
    def add_simplex(self, simplex: Iterable[ElementType] | ElementType, **attr) -> None:
    ...
    def to_trimesh(self, vertex_position_name: str = "position"):
        ...
        faces = list(
            map(lambda simplex: simplex.elements, self.get_all_maximal_simplices())
        )
