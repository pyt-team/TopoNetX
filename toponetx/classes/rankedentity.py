...
class RankedEntity(Entity):
   ...
   - All descendents of RankedEntity are registered in :func:`RankedEntity.fullregistry()`.    
    ...
    def __init__(self, uid , elements=[], rank=None, rankedentity=None, weight=1.0, safe_insert=True, **props):
        ...
        if isinstance(elements, dict): # often used with RankedEntitySet
            ...
            for k in ordered_keys:
                if isinstance(elements[k], RankedEntity):
                    self.add_element(elements[k],safe_insert = safe_insert,check_CC_condition=True)
                    self._all_ranks=self._all_ranks.union(elements[k]._all_ranks)
                elif 'elements' in elements[k] and 'rank' in elements[k]:
                    self.add_element( (k, elements[k] ) ,safe_insert = safe_insert,check_CC_condition=False)
                    self._all_ranks.add(elements[k]['rank'] )
                elif isinstance(elements[k],tuple) or isinstance(elements[k],list): 
                    assert( len(elements[k])==2) # elements[k] must of the form (cell,rank) 
                    assert(isinstance(elements[k][1],int)) # assert second element,rank is an integer
        ...
        if safe_insert:  
            assert( isinstance (self._rank,int) or self._rank == np.inf ) # rank must be an integer 
            assert( self._rank >= 0 ) # rank must be non-negative integer 
            if self.depth() == 0 and not isinstance( self, RankedEntitySet) : # depth of the current entity in the poset structure 
                assert (self._rank == 0) # rank must be zero for primitive entities
            if self._rank == 0:
                assert( len(self) == 0) # zero ranked entity must be not contain any other entity. 
            elif not isinstance( self, RankedEntitySet):
                assert(self._rank > 0) # rank must be a positive integer for non-primitive entities
    ...
    def add_element(self, item,safe_insert=True,check_CC_condition=False):
        ...
        if isinstance(item, RankedEntity):
            ...
            if self.uid == item:
            ...
            elif isinstance(item, tuple): # e.g. item=(key, {"element":[1,2,3],"rank":3 }  )
                if isinstance(item[1],dict):
                    ...
                    if self.uid == item[0]:
                        ...
                        for i in item[1]['elements']:
                            if i in checkelts :
                                elem.append(checkelts[i]) # element is an existing RankedEntity
                            else:
                                elem.append(i) # element is a primitive entity
                        ent = RankedEntity( item[0], elements=elem, 
                ...
                if item in checkelts:
                    ...
                    self._elements[item] = RankedEntity(item, _memberships={self.uid: self},rank = 0)
    ...
    def incidence_dict(self):
        ...
        temp= OrderedDict([ (ent,{"elements": registry[ent].uidset , "rank": registry[ent].rank }) for ent in  ordered_keys ])
...
class RankedEntitySet(RankedEntity):
    ...
    def __init__(self, uid, elements=[], safe_insert=True, **props):
        ...
        super().__init__(uid=uid, elements=elements, rank=np.inf, safe_insert=safe_insert, **props)
    ...
    def _incidence_matrix_helper(self,children,uidset,sparse=True, index=False):
       ...
       --------
        ...
        r_cell_dict = {
                j: children[j].skeleton(0).uidset  for j in children
            }
        k_cell_dict = {
                i: uidset[i].skeleton(0).uidset  for i in uidset
            }
                ...
                for e in k_cell_dict:
                    for n in r_cell_dict:
                        if  r_cell_dict[n] <= k_cell_dict[e]  :                   
                            ...
                            rows.append(ndict[n])
                            cols.append(edict[e])
                MP = csr_matrix((data, (rows, cols)),shape=( len(r_cell_dict), len(k_cell_dict)  ))
            else:
                # Create an np.matrix
    ...
    def incidence_matrix(self,r,k, incidence_type = "up", weight=None, sparse=True, index=False):
        ...
        if k is None:
            ...
            elif r>k: # up incidence is defined between two skeletons of different ranks
    ...
    def adjacency_matrix(self,r, k, s=1, weights=False,index=False ):
        ...
        if index:
            ...
            MP, row, col = self.incidence_matrix(r,k, incidence_type='up', sparse = True,index = index)
        else:
            MP = self.incidence_matrix(r,k,incidence_type='up', sparse = True, index = index)
    ...
    def coadjacency_matrix(self,r,k, s=1, weights=False,index=False ):
        ...
        if index:
            ...
            MP, row, col = self.incidence_matrix(r,k,incidence_type='down', sparse = True, index=index)
        else:
            MP = self.incidence_matrix(k,r,incidence_type='down', sparse = True, index=index)
        weights = False ## currently weighting is not supported
        if weights == False:
            A = MP.T.dot(MP)
            A.setdiag(0)
    ...
    def collapse_identical_elements(self, newuid, return_equivalence_classes=False):
        ...
        uniqe_reps={} # fix the reps across, this will be used when returning the RankedEntitySet
