...
TopoNetX tests can be run using `pytest <https://docs.pytest.org/>`_.
...
is a corner-stone of `TopoNetX` development process.
...
TopoNetX uses the `pytest` Python tool for testing different functions and features.
...
latest changes of the main toponetx repository. Bring remote changes locally:
...
#. **Submit your code with associated unit tests**. High-quality
   ...
   is a corner-stone of the toponetx development process.
...
#. **Make sure that your PR follows toponetx coding style and API** (see :ref:`coding-guidelines`). Ensuring style consistency throughout
   toponetx allows using tools to automatically parse the codebase,
